...
- **AI 知识助手**：基于你的知识库对话问答，回答 **附带引用来源**，聊天会话持久化
...
- **图书**：把 EPUB / PDF 直接拖入桌面端「图书角」，AI 在后台读正文、填字段；拖一次自动入库，重复文件自动去重
...
- [x] 发现页知识画像
- [x] 软删除 + 统一回收站 + 数据库备份/恢复
- [x] 扩展自动握手连接 + token 鉴权
- [x] 安全加固（CSP / 常量时间比较 / SSRF / 速率限制 / 输入验证）
- [x] **图书角**：EPUB / PDF 拖入 + AI 元数据抽取 + 封面展示
