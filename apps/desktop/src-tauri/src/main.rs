...
fn open_db() -> Result<Connection, String> {
    ...
    CREATE TABLE IF NOT EXISTS ai_topic (
      id    INTEGER PRIMARY KEY AUTOINCREMENT,
      name  TEXT NOT NULL UNIQUE,
      score INTEGER NOT NULL DEFAULT 0,  -- 0~100，表示相对强项分数
      descr TEXT
    );

    CREATE TABLE IF NOT EXISTS ai_note_topic_map (
      note_id  INTEGER NOT NULL,
...
fn migrate_plan_minutes_nullable(conn: &rusqlite::Connection) -> Result<(), String> {
    ...
    if !notnull {
        return Ok(()); // 已经是可空，无需处理
    }

    // 重建表：minutes 去掉 NOT NULL
    conn.execute_batch(
        r#"
    BEGIN;
...
#[tauri::command]
fn ai_classify_note_cloud(note_id: i64) -> Result<Vec<ClassifyHit>, String> {
    let mut conn = open_db()?;
    ...
    // 请求大模型
    let pairs = ai_pick_skills(&text, &cfg)?;
...
fn ai_analyze_topics() -> Result<Vec<AiTopicOut>, String> {
    ...
    if notes_cnt == 0 {
        ...
        tx.execute("DELETE FROM ai_topic", []).ok();
        tx.commit().ok();
        return Ok(vec![]);
    }
    ...
    let sys = "你是资深学习教练。请阅读用户的笔记列表，提炼 8 个最能代表其能力的主题（不重复，中文名简洁）。\
输出严格 JSON，格式如下：\
{\"topics\":[{\"name\":\"主题A\",\"note_ids\":[1,2]}, ... 共 8 项]}。\
只能输出 JSON，不要多余文字。";
    let usr = format!("笔记列表(最近 200 条)：[\n{}\n]", note_list);
    ...
    for t in topics {
        ...
        for nid in note_ids {
            ...
            tx.execute(
                "INSERT OR IGNORE INTO ai_note_topic_map(note_id, topic_id, weight) VALUES (?1, ?2, 1)",
                rusqlite::params![nid, topic_id],
            ).map_err(|e| e.to_string())?;
        }
    }
...
fn get_plan_goal() -> Result<String, String> {
    let conn = open_db()?;
    Ok(kv_get(&conn, "plan_goal")?.unwrap_or_default())
}

#[tauri::command]
fn set_plan_goal(goal: String) -> Result<(), String> {
    let conn = open_db()?;
    conn.execute(
