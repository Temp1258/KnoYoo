...
export default function ClipsTrashPanel({ onCountChange }: Props) {
  ...
  // Single source of truth for the tab-label count — derived from trashCount.
  // Using an effect avoids stale-closure bugs when multiple handlers fire
  // concurrently (each would otherwise read its own snapshot of trashCount).
  useEffect(() => {
    ...
    <div>
      ...
      {!loading && clips.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-text-tertiary">
          ...
          <p className="text-[15px] font-medium mb-1">没有已删除的剪藏</p>
          <p className="text-[12px]">删除的剪藏会出现在这里</p>
        </div>
      )}
